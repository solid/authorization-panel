...
ex:PolicyX
    acp:allow acl:Read ; # Sarven: would only having acp:allow acl:Read imply anything other than Read? Why would acp:deny be necessary?
    acp:deny acl:Write ;
...
**Matthieu :**
Response to Sarven's comment on the necessity of `acp:deny`:
