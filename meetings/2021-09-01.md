...
Read and Write a for sure good. Indeed, I have [implemented a Web Server](https://github.com/bblfish/lens-play/blob/master/src/main/scala/server/Server.scala) using 
a very simple data structure for containers, together with a concept from Functional Programming 
called Lenses. Lenses are essentially built out of two types of operations: getter and setters;
or Read and Write operations. With those operation one can build up all
of LDP. 
But I think Control is in the wrong place. I don't see it as type of mode, I see it as relation.
...
**Sarven** In WAC we treat ACL as data points as well. Even possibility of ACL having ACL suggests that they should be considered as data point. 
I agree that it can be achieved in a different way. The aspect of operations has been in the protocol for quite a while. 
Operations are not just atomic modes but class of operations e.g. write includes create and delete. See https://github.com/solid/web-access-control-spec/issues/85 .
I talked to Kjetilk about why create the operation issue under authorization-panel - it was to create engagement. Protocol has some of those definitions 
...
The reason is eg. PUT typically needs Write mode, but PUT + If-None-Match: * would need Write and Read.
...
**Justin** having Update / Modify on container means for me that one can change the non-server managed (containement) triples. To change those one would need Created (which would result in adding) and Delete (removing)

**Sarven** I'd like to see actual applications before hypothesising.

**Henry** I think we should take some use cases, try to implement them as we have done in in the evaluation
section https://github.com/solid/authorization-panel/tree/main/proposals/evaluation
...
When it comes to dogfooding it hard if current systems don't support it. We do have use cases which detail Create, Delete, Update and real world cases when current access modes lead to over permissioning. I would like to understand what else is requested besides use cases that are already included.
