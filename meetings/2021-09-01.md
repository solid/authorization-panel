...
Operations are not just atomic modes, but classes of operations; e.g., write includes create and delete. See https://github.com/solid/web-access-control-spec/issues/85 .
...
The reason is, e.g., `PUT` typically needs only `Write` mode, but `PUT + If-None-Match: *` would need both `Write` and `Read`.
