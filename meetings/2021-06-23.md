...
Aaron: +1 to @justinwb 's comment about not wanting to go directly to VC --- the use cases need to be grounded in both general cases (e.g. not with VCs) and also with specific cases (e.g. with VCs). But packing everything into a single use case complicates things.
