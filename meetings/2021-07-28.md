...
There have been many experiments in giving the user back control of their data that make this mistake: they aim to put the user at the center, but then fail to make a useable protocol that makes exchanging data easy. For example, the original [mes infos](http://mesinfos.fing.org) project I recently mentioned, designed for personal cloud computing in France had this type of architecture. The user could save his own data, but the protocol for exchanging this with anyone else was some JSON RPC style protocol.
...
Henry: If you put up this question, we can put up a Use Case and have a user story where this is an issue and show how the different systems work. But out of the blue like that, we do not have anything that can be constructively discussed. 

 It seems overly paranoid to me as a UC. That is to discuss giving out one bit of information - whether some resource exists or not - when we do not have a system that is working in allowing us to cooperate across the web, is emphasizing the wrong thing. As I mentioned above, at present, most people are losing terabytes of information to large companies that have set up men MiM services. 
